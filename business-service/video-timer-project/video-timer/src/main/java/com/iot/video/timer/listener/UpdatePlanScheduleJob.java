...
public class UpdatePlanScheduleJob implements Job {
	...
	public void execute(JobExecutionContext jobExecutionContext) throws JobExecutionException {
		...
		messageApi = (MessageApi)(jobExecutionContext.getMergedJobDataMap().get("messageApi"));
		userApiService = (UserApi)(jobExecutionContext.getMergedJobDataMap().get("userApiService"));
		deviceCoreApi = (DeviceCoreApi) (jobExecutionContext.getMergedJobDataMap().get("deviceCoreApi"));
		ipcApi = (IpcApi) (jobExecutionContext.getMergedJobDataMap().get("ipcApi"));
		goodsServiceApi = (GoodsServiceApi)(jobExecutionContext.getMergedJobDataMap().get("goodsServiceApi"));
		langApi = (LangApi) (jobExecutionContext.getMergedJobDataMap().get("langApi"));
		try {
	...
	public void sendMail(SchedulePlanDto schedulePlan,String templateId,String subjectSuffix){
		...
		try {
			...
			userIds.add(schedulePlan.getUserId());
			List<UserPhoneRelate> userPhoneRelates = messageApi.getUserPhoneRelates(userIds);
			if(userPhoneRelates != null && userPhoneRelates.size() > 0) {
				...
				messageApi.mailSinglePush(userPhoneRelates.get(0).getAppId(), user.getUserName(), map, 0, LocaleContextHolder.getLocale().toString());
			}
		} catch (Exception e) {
