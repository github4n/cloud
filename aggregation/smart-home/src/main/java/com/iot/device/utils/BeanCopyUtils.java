...
public class BeanCopyUtils {
    ...
    public static void copyDeviceStatusList(Map<String, ListDeviceStatusRespVo> tempMap, List<DeviceResp> targetList) {
        ...
        for (DeviceResp target : targetList) {
            ...
            ListDeviceStatusRespVo deviceType = tempMap.get(target.getDeviceTypeId());
            if (deviceType == null) {
                i++;
                continue;
            }
            target.setOnlineStatus(deviceType.getOnlineStatus());
            target.setActiveStatus(deviceType.getActiveStatus());
            targetList.set(i, target);
            i++;
        }
    }
