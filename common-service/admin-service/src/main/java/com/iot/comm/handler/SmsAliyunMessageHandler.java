...
import com.iot.comm.config.SmsAliyunPropertiesConfig;
 ...
 * 阿里大鱼短息服务处理
 */
@Component(CommConstants.ALIYUN_SMS)
public class SmsAliyunMessageHandler extends AbstractMessageHandler {

    private static final Logger LOGGER = LoggerFactory.getLogger(SmsAliyunMessageHandler.class);
...
//            LOGGER.info("短信发送完毕，手机号：{}，返回状态：{}", mobileMsgTemplate.getMobile(), sendSmsResponse.getCode());
//        } catch (ClientException e) {
//            LOGGER.error("发送异常");
//            e.printStackTrace();
//        }
    ...
    public void fail(MobileMsgTemplate mobileMsgTemplate) {
        LOGGER.error("短信发送失败 -> 网关：{} -> 手机号：{}", mobileMsgTemplate.getType(), mobileMsgTemplate.getMobile());
    }
}
