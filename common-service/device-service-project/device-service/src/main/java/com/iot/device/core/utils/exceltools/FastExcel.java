package com.iot.device.core.utils.exceltools;

import com.iot.device.comm.utils.exceltools.annotation.GetModelNameAndType;
...
public final class FastExcel implements Closeable {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(com.iot.device.comm.utils.exceltools.FastExcel.class);
    /**
     * 时日类型的数据默认格式化方式
     */
...
//        }
        ...
        try {
            ...
            if (null != sheet) {
                ...
                for (int i = this.startRow + 1; i <= sheet.getLastRowNum(); i++) {
                    ...
                    if (null != t && GetModelNameAndType.reflectBool(t)) {
                        resultList.add(t);
                    }
                }
            } else {
    ...
    public static String getWebRootPath(String filePath) {
        try {
            String path = com.iot.device.comm.utils.exceltools.FastExcel.class.getClassLoader().getResource("").toURI().getPath();
